          ...
          {toUIMessages(messages.results ?? [])?.map((message) => {
            return (
              <AIMessage from={message.role === "user" ? "user" : "assistant"} key={message.id}>
                ...
                {message.role === "assistant" && <DicebearAvatar imageUrl="/logo.svg" seed="assistant" size={32} />}
              </AIMessage>
            );
          })}
        </AIConversationContent>
          ...
          <FormField
            ...
            render={({ field }) => (
              <AIInputTextarea
                ...
                placeholder={conversation?.status === "resolved" ? "This conversation has been resolved" : "Type your message here..."}
                value={field.value}
              />
            )}
          />
