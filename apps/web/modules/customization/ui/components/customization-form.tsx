...
export const CustomizationForm = ({ initialData, hasVapiPlugin }: CustomizationFormProps) => {
  ...
  const form = useForm<FormSchema>({
    ...
    defaultValues: {
      greetMessage: initialData?.greetMessage || "Hi! How can we help you today?",
      defaultSuggestions: {
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea {...field} placeholder="Welcome message shown when chat open" rows={3} />
                  </FormControl>
                  <FormDescription>The first message customers see when they open the chat widget.</FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            />
